...
class CSimplate {
    ...
    function CSimplate($psHtmlPath, $psPHPPath = null, $psplMaster = null, $psEntryPoint = "") {
        ...
        if (!defined('SPL_FORCE'))
            define('SPL_FORCE', false);
        if (!defined('SPL_PROJECT_NAME'))
            define('SPL_PROJECT_NAME', 'Silos Framework');
    ...
    function mountPhpPage(&$pobjDom = null, $pideep = 0, $pbFlag = false) {
        ...
        if ($pobjDom instanceof simple_html_dom_node) {
            if ($pbFlag)
                return $this->mountPhpNode($pobjDom, $pideep, $pbFlag);
            $this->mountPhpNode($pobjDom, $pideep, $pbFlag);
        }
        foreach ($pobjDom->childNodes() as $childNode) {
            if ($pbFlag)
                return $this->mountPhpPage($childNode, $pideep, $pbFlag);
    ...
    function mountPhpNode(&$objNode, $pideep = 0, $pbFlag = false) {
        ...
        if (isset($objNode->scope))
            $sScope = $objNode->scope;
        $objNode->__unset('scope');

        if ($objNode->__isset('show'))
            $this->commandShow($objNode, $sScope);

        /* Command bind */
        if (!$sScope)
            $sScope = 'inner'; // Default value to the scope (bind)
        if ($objNode->__isset('bind')) {
            ...
            if (isset($objNode->as))
                $sScript .= $this->commandBindAs($objNode, $pideep, $sScope);
            else
                $sScript .= $this->commandBind($objNode, $pideep);

            if ($sScope != "tag")
                $objNode->innertext = $sScript;
            $temScape = $this->replaceAttributes($objNode, 'temp_' . $pideep);
            if ($sScope == "outer") {
                if (!$temScape)
                    $objNode->outertext = '<?php $temp_' . $pideep . '=$' . $sCurrObjName . ';?>' . $sScript;
                else
                    $objNode->outertext = '<?php $temp_' . $pideep . '=$' . $sCurrObjName . ';if ($temp_' . $pideep . '){ ?>' . $objNode->outertext . '<?php } ?>';
            }
            else
                $objNode->outertext = '<?php $temp_' . $pideep . '=$' . $sCurrObjName . ';if ($temp_' . $pideep . '){ ?>' . $objNode->outertext . '<?php } ?>';
        }
    ...
    function commandShow($pobjNode, $psScope) {
        if (!$psScope)
            $psScope = 'outer'; // Default value to the scope (show)
        ...
        if ($psScope == "outer")
            $objAux->load($pobjNode->outertext);
        else if ($psScope == "inner")
            $objAux->load($pobjNode->innertext);
    ...
    function commandBindAs($pobjNode, $pideep, $psScope = 'inner') {
        ...
        if ($psScope == 'outer')
            $sScript .=$pobjNode->outertext;
        else
            $sScript .= $pobjNode->innertext;
        if ($iLimit) {
            $sScript .= ' <?php } ?>' . "\n";
        }
        $sScript .= ' <?php } ?>' . "\n";
